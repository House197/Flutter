...
class _SizeSelector extends StatelessWidget {
  ...
  final void Function(List<String> selectedSized) onSizesChanged;
    ...
    return Center(
      child: SegmentedButton(
        ...
        segments: genders.map((size) {
          return ButtonSegment(icon: Icon(genderIcons[genders.indexOf(size)]), value: size, label: Text(size, style: const TextStyle(fontSize: 12)));
        }).toList(),
  ...
  Widget build(BuildContext context) {
    return PageView(
      ...
      children: images.isEmpty
          ? [
              ClipRRect(
                  borderRadius: const BorderRadius.all(Radius.circular(20)), child: Image.asset('assets/images/no-image.jpg', fit: BoxFit.cover))
            ]
          : images.map((e) {
              return ClipRRect(
